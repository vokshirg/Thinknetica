...
class RailWayManager
  ...
  def ui
    ...
    while comand != 9
      ...
      case comand
      when 1 then create_st
      when 2 then create_train
      when 3 then add_wagon
      when 4 then del_wagon
      when 5 then tr_to_st
      when 6 then puts st
      when 7 then puts tr
      when 8 then puts "В данном поезде #{train_by_num.wagons.length} вагонов(а)"
      when 9 then break
      else
  ...
  def create_train
    ...
    while type != 1 && type != 2
      ...
      puts "Вы указали неправильный тип" if type != 1 && type != 2
    end
